...
describe('User', async function () {
    ...
    describe('#createUser', async function () {
        ...
        it('Should create a new user', async function () {
            await testCreateUser(CONST.JOAN_CLARKE, 'jclarke', 'enigmamachine', null);
        });
        it('Should create user with mail', async function () {
            await testCreateUser(CONST.ADA_LOVELACE, 'alovelace', 'mathiscool', CONST.ALOVELACE_AT_MATH_DOT_GOV);
        });
        ...
        it('Should fail if no username is specified', async function () {
            await testCreateUserFailure(CONST.JOAN_CLARKE, null, null, null, "NO_USERNAME", "You must supply a username of 1-128 chars");
        });
        it('Should fail if username is taken', async function () {
            await testCreateUserFailure(CONST.JOAN_CLARKE, 'jclarke', null, null, "DUPLICATE_USERNAME", "Username is taken");
        });
        it('Should fail if password is too short', async function () {
            await testCreateUserFailure(CONST.JOAN_CLARKE, 'clarke', 'enigma', null, "INVALID_PASSWORD", "Invalid password (must be at least 8 chars)");
        });
        it('Should fail if e-mail is invalid', async function () {
            await testCreateUserFailure(CONST.JOAN_CLARKE, 'clarke', null, 'jclark@enigma', "INVALID_EMAIL", "Invalid e-mail (max 128 chars)");
        });
